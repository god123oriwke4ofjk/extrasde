...
command -v pacman >/dev/null 2>&1 || error_exit "pacman not found. This script requires Arch Linux."
...
[ ! -f "$BRAVE_SOURCE_DIR/$BRAVE_DESKTOP_FILE" ] && error_exit "$BRAVE_DESKTOP_FILE not found in $BRAVE_SOURCE_DIR."
...
if grep -q "font-family: 'Alef'" "$DCOL_FILE"; then
    ...
    if grep -q "/* Any custom CSS below here */" "$DCOL_FILE"; then
        sed -i "/\/* Any custom CSS below here *\/$/a\\$CUSTOM_CSS" "$DCOL_FILE" || error_exit "Failed to add custom CSS to $DCOL_FILE."
        echo "Added custom CSS to $DCOL_FILE after marker comment"
        echo "[$(date)] MODIFIED_CONFIG: $DCOL_FILE -> Added custom CSS" >> "$LOG_FILE"
    else
        echo "$CUSTOM_CSS" >> "$DCOL_FILE" || error_exit "Failed to append custom CSS to $DCOL_FILE."
        echo "Appended custom CSS to $DCOL_FILE (marker comment not found)"
        echo "[$(date)] MODIFIED_CONFIG: $DCOL_FILE -> Appended custom CSS (no marker)" >> "$LOG_FILE"
    fi
    if grep -q "font-family: 'Alef'" "$DCOL_FILE"; then
